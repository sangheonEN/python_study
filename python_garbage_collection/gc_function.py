...
Object Allocator(pymalloc): 요청된 객체 메모리 크기가 512 byte 미만일 때, Object Allocator 를 사용하여 할당한다.
...
- 순환 참조 문제란? 서로 다른 두 객체가 서로를 참조하여, 가비지 컬렉션 대상에 포함되지 않아 해당 객체들이 불필요해져도 메모리 회수되지 않는 문제 [2]

- gc.collect()를 활용하면, 순환 참조 문제까지 자동으로 해결할 수 있다.
